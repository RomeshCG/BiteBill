...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  const { user_id, filter, team_id } = req.query;
  if (!user_id) return res.status(400).json({ error: "Missing user_id" });

  let bills = [];
  let query = supabase.from("receipts").select("id, team_id, title, created_at, teams(name), receipt_items(amount)");
  ...
  if (filter === "paid") {
    ...
    bills = (receipts || []).map((r: any) => ({
      team: r.teams?.name || r.team?.[0]?.name || "",
    ...
    // Bills where user is a payer and their split is settled = true
